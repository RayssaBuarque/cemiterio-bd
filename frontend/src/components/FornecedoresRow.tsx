...
interface FornecedorRowProps extends IFornecedorInput {
    isEven: boolean;
    updateList: () => void;
}

const FornecedorRow = ({ cnpj, nome, endereco, isEven, updateList }: FornecedorRowProps) => {
    
    const [isModalOpen, setisModalOpen] = useState(false);
    const { register, handleSubmit } = useForm<IFornecedorInput>();

    const handleUpdate = async (data: IFornecedorInput) => {
        try {
            // await api.updateFornecedor(cnpj, data); // Exemplo de chamada
    ...
    const handleDelete = async () => {
        ...
        try {
            // await api.deleteFornecedor(cnpj);
            console.log("Deletando fornecedor:", cnpj);
            await updateList();
            setisModalOpen(false);
        } catch (error) {
    ...
    return (
            ...
            <ModalOverlay onClick={() => setisModalOpen(false)}>
                <ModalContainer onClick={(e) => e.stopPropagation()}>
                    ...
                    <form onSubmit={handleSubmit(handleUpdate)}>
                        <MainPopUp>
                            <FormContainer>
                                {/* Linha 1: CNPJ (Geralmente imutável) e Nome */}
        ...
        {/* Linha da Tabela - Deve bater com FornecedoresGrid (1fr 2fr 3fr) */}
...
// Grid: 1fr 2fr 3fr (CNPJ | Nome | Endereço)
